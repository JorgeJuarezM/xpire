...
class Intel8080(CPU):
    ...
    def compare_with_twos_complement(self, v1: int, v2: int) -> int:
        compl = get_twos_complement(v2)  # & 0xFF
        result = v1 + compl
    ...
    @manager.add_instruction(0xA1, [Registers.A, Registers.C])
    ...
    def decrement_memory_byte(self) -> None:
        ...
        value = self.read_memory_byte(address)
        result = self.decrement_byte_value(value)
    ...
    def substract_immediate_from_accumulator_with_borrow(self) -> None:
        i = self.fetch_byte() + self.flags.C
        result = self.substract_with_twos_complement(
            self.registers[Registers.A],
            i,
        )
    ...
    @manager.add_instruction(OPCodes.ADC_M)
    def add_memory_to_accumulator_with_carry(self) -> None:
        value1 = self.registers[Registers.A]
        h = self.registers[Registers.H]
        l = self.registers[Registers.L]
        value2 = self.read_memory_byte(join_bytes(h, l))
    ...
    # @manager.add_instruction(0x54)
    # @manager.add_instruction(0x5D)
    # @manager.add_instruction(0x0B)
    # @manager.add_instruction(0x12)
    @manager.add_instruction(0x28)
    def exec_no_operation(self):
        return super().exec_no_operation()
